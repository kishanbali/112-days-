...
async function hashAccessToken(env, token) {
  return sha256Hex(`${env.ACCESS_TOKEN_PEPPER}:${token}`);
}
...
async function createOrder(request, env) {
  ...
  const price = await env.DB.prepare(
    'SELECT stage_key, amount_paise, currency FROM stage_prices WHERE stage_key = ? AND active = 1'
  ).bind(stageKey).first();
  if (!price || !Number.isInteger(price.amount_paise) || price.amount_paise <= 1) {
    return json({ ok: false, error: 'Stage price is not configured for checkout.' }, 409);
  }

  const checkoutToken = randomToken(32);
  const checkoutHash = await sha256Hex(checkoutToken);
  const order = await razorpay('/orders', env, {
    method: 'POST',
    body: JSON.stringify({
      amount: price.amount_paise,
      currency: price.currency,
      receipt: `kemp-${stageKey}-${now()}-${crypto.randomUUID().slice(0, 8)}`,
      notes: { kemp_stage: stageKey }
    })
  });

  await env.DB.prepare(`
    INSERT INTO orders(order_id, stage_key, amount_paise, currency, checkout_token_hash, status, created_at)
    VALUES(?, ?, ?, ?, ?, 'created', ?)
  `).bind(order.id, stageKey, price.amount_paise, price.currency, checkoutHash, now()).run();
...
async function verifyPayment(request, env) {
  ...
  if (!checkout_token || !razorpay_payment_id || !razorpay_order_id || !razorpay_signature) {
  ...
  const order = await env.DB.prepare(
    'SELECT * FROM orders WHERE checkout_token_hash = ? AND order_id = ?'
  ).bind(checkoutHash, razorpay_order_id).first();
  if (!order) return json({ ok: false, error: 'Order not found.' }, 404);
  if (order.status === 'paid' && order.access_token_hash) {
    return json({ ok: true, stage_key: order.stage_key, already_paid: true });
  }
  ...
  if (payment.order_id !== razorpay_order_id || payment.status !== 'captured') {
    return json({ ok: false, error: 'Payment is not captured for this order.' }, 409);
  }
  if (payment.amount !== order.amount_paise || payment.currency !== order.currency) {
    return json({ ok: false, error: 'Payment amount/currency mismatch.' }, 409);
  }

  const accessToken = randomToken(32);
  const accessHash = await hashAccessToken(env, accessToken);
  await env.DB.prepare(`
    UPDATE orders SET status='paid', payment_id=?, access_token_hash=?, paid_at=?
    WHERE order_id=? AND status != 'paid'
  `).bind(razorpay_payment_id, accessHash, now(), razorpay_order_id).run();
...
async function webhook(request, env) {
  ...
  if (eventName === 'payment.captured' || eventName === 'order.paid') {
    ...
    if (orderId) {
      ...
      if (order && order.status !== 'paid' && paymentEntity?.status === 'captured') {
        if (paymentEntity.amount === order.amount_paise && paymentEntity.currency === order.currency) {
          const accessToken = randomToken(32);
          const accessHash = await hashAccessToken(env, accessToken);
          await env.DB.prepare(`
            UPDATE orders SET status='paid', payment_id=?, access_token_hash=?, paid_at=?
            WHERE order_id=? AND status != 'paid'
          `).bind(paymentId || null, accessHash, now(), orderId).run();
        }
      }
    }
  }
  ...
  await env.DB.prepare(
    'INSERT INTO processed_events(event_id, event_name, processed_at) VALUES(?, ?, ?)'
  ).bind(eventId, eventName || 'unknown', now()).run();
...
async function access(request, env) {
  ...
  const order = await env.DB.prepare(
    'SELECT stage_key, status FROM orders WHERE access_token_hash = ? AND status = \'paid\''
  ).bind(hash).first();
